...
router.get('/', upload.none(), function(req, res) { //make this into a choice between bots, 2p or 4p
...
router.post('/validate',upload.none(), function(req, res) {
  ...
  if(sess.userObj){
    ...
    if(!(req.body.isPrivate && roomFunctions.checkRoomPassword(roomName, pass) 
        && roomFunctions.checkIfExists(roomName))){ //fail
          err = 419;
    }
    else{
