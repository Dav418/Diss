var gameTiles = [
    ...
    {id:15, type: "prop-station", name:"placeholder 11", cost:200, owned:-1},
    ...
    {id:25, type: "prop-station", name:"placeholder 18", cost:200,owned:-1},
    ...
    {id:35, type: "prop-station", name:"placeholder 26", cost:200,owned:-1},
...
function checkPlayerTile(room, place, playerName){
    ...
    var tilePlayerIsOn = figureOutWhatTileThePlayerIsOn(place);
    
    switch(tilePlayerIsOn.type){
        case "special":
            // special
            var socketio = require("../socketFiles/socketFunctions.js");
            var data = {playerName: playerName, type:"special" + tilePlayerIsOn.name}
        ...
        case "prop":
            if(tilePlayerIsOn.owned == -1){
                ...
                var socketio = require("../socketFiles/socketFunctions.js");
                var data = {playerName
                    : playerName, name: tilePlayerIsOn.name}
                socketio.sockets.to(room.roomName).emit("player_tile_buy", data)
            }else{//someone does own it
        ...
        case "chest":
            //do chest stuff
            var socketio = require("../socketFiles/socketFunctions.js");
            var data = {playerName:playerName, type:"chest  " + tilePlayerIsOn.name}
            socketio.sockets.to(room.roomName).emit("player_tile", data)
            break;
        case "prop-station":
            //do station stuff
            var socketio = require("../socketFiles/socketFunctions.js");
            var data = {playerName: playerName, type:"prop-station " + tilePlayerIsOn.name}
            socketio.sockets.to(room.roomName).emit("player_tile", data)
            break;
        case "jail":
            //send player to jail
            var socketio = require("../socketFiles/socketFunctions.js");
            var data = {playerName: playerName, type:"jail " + tilePlayerIsOn.name}
            socketio.sockets.to(room.roomName).emit("player_tile", data)
            break;
        case "prop-stat":
            //do staion stuff
            var socketio = require("../socketFiles/socketFunctions.js");
            var data = {playerName: playerName, type:"prop-stat " + tilePlayerIsOn.name}
            socketio.sockets.to(room.roomName).emit("player_tile", data)
            break;
...
function figureOutWhatTileThePlayerIsOn(pos){
    var t;
...
module.exports={
    tileChecker:(room, who,place, playerName)=>{
        checkPlayerTile(room, who,place, playerName)
    },
